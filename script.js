...
class Character {
    constructor(x, y, color) {
        ...
        this.width = TILE_SIZE * 0.7; this.height = TILE_SIZE * 0.7;
        this.isDying = false; this.deathAnimTimer = 0;
        this.speed = 2; this.animFrame = 0; this.animTimer = 0;
    }
    ...
    draw() {
        ...
        ctx.fillRect(this.x + (this.width - headSize) / 2, this.y - 5 + bob, headSize, headSize);
        // Eyes
        ctx.fillStyle = 'black';
        ctx.fillRect(this.x + this.width * 0.3 - 2, this.y - 2 + bob, eyeSize, eyeSize);
        ctx.fillRect(this.x + this.width * 0.7 - 2, this.y - 2 + bob, eyeSize, eyeSize);
    }
    
    drawDeath() {
        this.deathAnimTimer++;
        const t = this.deathAnimTimer;
        const centerX = this.x + this.width/2;
        const centerY = this.y + this.height/2;
        if (t > 60) return;
        ctx.globalAlpha = 1 - t/60;
        ctx.fillStyle = '#fef3c7';
        ctx.beginPath(); ctx.arc(centerX, centerY, t*0.8, 0, Math.PI * 2); ctx.fill();
        ctx.fillStyle = this.color;
        ctx.beginPath(); ctx.arc(centerX, centerY, t*0.5, 0, Math.PI * 2); ctx.fill();
        ctx.globalAlpha = 1;
    }
...
class PowerUp {
    ...
    draw() { const colors = { [POWERUP_TYPE.BOMB]: '#60a5fa', [POWERUP_TYPE.FIRE]: '#f87171', [POWERUP_TYPE.SPEED]: '#4ade80' }; ctx.fillStyle = '#1e293b'; ctx.beginPath(); ctx.arc(this.x + this.size/2, this.y + this.size/2, this.size/2, 0, Math.PI*2); ctx.fill(); ctx.fillStyle = colors[this.type]; ctx.font = `${this.size * 0.6}px "Press Start 2P"`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; const text = { [POWERUP_TYPE.BOMB]: 'B', [POWERUP_TYPE.FIRE]: 'F', [POWERUP_TYPE.SPEED]: 'S' }[this.type]; ctx.fillText(text, this.x + this.size/2, this.y + this.size/2 + 2); }
    apply(player) { if (this.type === POWERUP_TYPE.BOMB) player.maxBombs++; if (this.type === POWERUP_TYPE.FIRE) player.bombRange++; if (this.type === POWERUP_TYPE.SPEED) player.speed = Math.min(player.speed + 0.5, 4); updateInfoPanel(); }
}
...
function drawMap() { for (let r = 0; r < MAP_ROWS; r++) { for (let c = 0; c < MAP_COLS; c++) { const x = c * TILE_SIZE; const y = r * TILE_SIZE; ctx.fillStyle = '#4a752c'; ctx.fillRect(x, y, TILE_SIZE, TILE_SIZE); if (gameMap[r][c] === TILE_TYPE.HARD_WALL) { ctx.fillStyle = '#78716c'; ctx.fillRect(x, y, TILE_SIZE, TILE_SIZE); ctx.fillStyle = '#a8a29e'; ctx.fillRect(x + 2, y + 2, TILE_SIZE - 4, TILE_SIZE - 4); } else if (gameMap[r][c] === TILE_TYPE.SOFT_WALL) { ctx.fillStyle = '#a16207'; ctx.fillRect(x, y, TILE_SIZE, TILE_SIZE); ctx.fillStyle = '#facc15'; ctx.fillRect(x + 2, y + 2, TILE_SIZE - 4, TILE_SIZE - 4); } } } }

function init() { isGameOver = false; generateMap(); powerUps = []; player = new Player(TILE_SIZE * 1.1, TILE_SIZE * 1.1); bombs = []; explosions = []; bots = [new Bot(TILE_SIZE * (MAP_COLS - 2.9), TILE_SIZE * 1.1), new Bot(TILE_SIZE * 1.1, TILE_SIZE * (MAP_ROWS - 2.9)), new Bot(TILE_SIZE * (MAP_COLS - 2.9), TILE_SIZE * (MAP_ROWS - 2.9)),]; keys = {}; updateInfoPanel(); gameOverScreen.classList.add('hidden'); gameLoop(); }
function update(deltaTime) { if (isGameOver) return; const dangerMap = new Set(); bombs.forEach(bomb => { calculateExplosionTiles(bomb.col, bomb.row, bomb.range).forEach(tileKey => dangerMap.add(tileKey)); }); player.update(); bots.forEach(bot => bot.update(dangerMap)); bombs.forEach(bomb => bomb.update(deltaTime)); explosions.forEach(exp => exp.update(deltaTime)); explosions.forEach(exp => { if (exp.checkCollision(player)) player.die(); bots.forEach(bot => { if (exp.checkCollision(bot)) bot.die(); }); }); }
...
function updateInfoPanel() { if (!player) return; livesDisplay.textContent = player.lives; bombsDisplay.textContent = player.maxBombs; rangeDisplay.textContent = player.bombRange; speedDisplay.textContent = player.speed.toFixed(1); messageDisplay.textContent = `Bots Remaining: ${bots.length}`; }
